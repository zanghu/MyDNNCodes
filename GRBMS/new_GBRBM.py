...
class HonglakLeeSparse(Cost):
    ...
    def get_gradients(self, model, data, ** kwargs):
        ...
        gradients = OrderedDict(izip([c], [1/self.p*gc]))
...
def get_visual(rbm_model, v_shape, start=0, stop=100, channels=1, save_fig=False, name=None, dpi=240):
    ...
    if channels == 1:
        ...
        w_max = numpy.max(W, axis=0)
        w_min = numpy.min(W, axis=0)
        W = W / (w_max - w_min)
    else: #如果是RGB图像，则应该将权值矩阵的每一列分解为三个channel，对每个channel做处理，即对每个channel
        W = W.reshape(pic_num, 3, v_shape[0], v_shape[1]) #.transpose(0, 2, 3, 1)
        w_max = numpy.max(W)
        w_min = numpy.min(W)
        for i in xrange(3):
            W[:, i, :, :] = W[:, i, :, :] / (w_max - w_min).sum(axis=0)
